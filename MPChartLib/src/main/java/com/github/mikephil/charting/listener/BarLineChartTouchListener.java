...
public class BarLineChartTouchListener extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<?
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                lastX = x;
                if (mTouchMode == DRAG) {
                    if (nestScrollStarted) {
                        performNestScroll((int)dx, (int)dy, event);
                        return true;
                    }
                    else {
                        float adx = Math.abs(dx);
                        float ady = Math.abs(dy);
                        if (ady > 2 * adx && (!mChart.isDragYEnabled())) {
                            nestScrollStarted = true;
                            performNestScroll((int)dx, (int)dy, event);
                            return true;
                        }
                        else {
    ...
    private void performDrag(MotionEvent event) {
        ...
        if (inverted()) {
            ...
            if (mChart instanceof HorizontalBarChart) {
                dX = -(x - mTouchStartPoint.x);
                dY = y - mTouchStartPoint.y;
            } else {
                dX = x - mTouchStartPoint.x;
                dY = -(y - mTouchStartPoint.y);
            }
        ...
        mMatrix = mChart.getViewPortHandler().getMatrixTouch();
    ...
    protected void performNestScroll(int dx, int dy, MotionEvent e) {

    }

    boolean nestScrollStarted = false;
    protected void performNestScrollStop() {
        nestScrollStarted = false;
    }
