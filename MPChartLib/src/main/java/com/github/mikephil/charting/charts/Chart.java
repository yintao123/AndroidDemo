...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
    ...
    private boolean markerViewForEachHighlight = false;
    ...
    protected OffsetListener offsetListener;
    /**
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        int len = mIndicesToHighlight.length;
        if (!markerViewForEachHighlight)
            len = 1;
        for (int i = 0; i < len; i++) {
    ...
    public void setMarkerViewForEachHighlight(boolean enable)
    {
        markerViewForEachHighlight = enable;
    }
    /**
     * Returns a recyclable MPPointF instance.
     *
    ...
    public interface OffsetListener {
        void offsetPrepared(float left, float top, float right, float bottom);
    }
}
