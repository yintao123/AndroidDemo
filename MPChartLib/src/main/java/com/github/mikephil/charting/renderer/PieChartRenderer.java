...
public class PieChartRenderer extends DataRenderer {
    ...
    public PieChartRenderer(PieChart chart, ChartAnimator animator,
        ...
        mHolePaint.setAlpha(5);

        mTransparentCirclePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mTransparentCirclePaint.setColor(Color.WHITE);
        mTransparentCirclePaint.setStyle(Style.FILL);
        mTransparentCirclePaint.setAlpha(0);
    ...
    public void drawData(Canvas c) {
        ...
        for (IPieDataSet set : pieData.getDataSets()) {
            ...
            if (set.isVisible() && set.getEntryCount() > 0)
            {
                ...
                if (pieData.pieHeight == 0)
                {
                    drawDataSet(c, set, false);
                }
                else {
                    boolean forShadow = true;
                    int level = pieData.pieHeight;
                    for (int i = 1; i <= level ; ++i)
                    {
                        if (i == level)
                            forShadow = false;
                        mBitmapCanvas.save();
                        float dt = 0.95f - i / 300f;
                        mBitmapCanvas.scale(1f, dt);
                        drawDataSet(c, set, forShadow);
    ...
    private void setColor(int color, boolean forShadow)
    {
        if (forShadow)
        {
            ...
            hsv[2] = hsv[2]*0.8f;
            int nc = Color.HSVToColor(hsv);
            mRenderPaint.setColor(nc);
        }
        else
        {
    ...
    protected void drawDataSet(Canvas c, IPieDataSet dataSet, boolean shadow) {
...
//                    float d = angle + rotationAngle + sliceAngle/2f;
//                    float shift = dataSet.getSelectionShift();
//                    float dx = shift * (float)Math.cos(d * Utils.FDEG2RAD);
//                    float dy = shift * (float)Math.sin(d * Utils.FDEG2RAD);
//                    mBitmapCanvas.save();
//                    mBitmapCanvas.translate(dx, dy);
//                }

                    final boolean accountForSliceSpacing = sliceSpace > 0.f && sliceAngle <= 180.f;

                    setColor(dataSet.getColor(j), shadow);
...
//                }
    ...
    public void drawValues(Canvas c) {
        ...
        if (data.pieHeight > 0)
        {
            float dt = 0.95f - data.pieHeight / 300f;
            c.scale(1f, dt);
        }
...
//        drawHole(c);
    ...
    public void drawTriangle(Canvas c) {
        int index = mChart.getPieChartItemIndex();
        MPPointF center = mChart.getCenterCircleBox();
        float radius = mChart.getRadius() * (mChart.getHoleRadius() / 100f);
        ...
        if(index == 0){
            ...
            angle = mChart.getAbsoluteAngles()[index]-(mChart.getAbsoluteAngles()[0]/2+90)-
                    (mChart.getAbsoluteAngles()[index] - mChart.getAbsoluteAngles()[index-1])/2;
        }
        float xTop = centerX + (radius+40f) * (float)Math.cos(angle * 3.14 /180);
        float xLeft = centerX + radius * (float)Math.cos((angle-5) * 3.14 /180);
        float xRight = centerX + radius * (float)Math.cos((angle+5) * 3.14 /180);
        float yTop = centerY + (radius+40f) * (float)Math.sin(angle * 3.14 /180);
        float yLeft = centerY + radius * (float)Math.sin((angle-5) * 3.14 /180);
        float yRight = centerY + radius * (float)Math.sin((angle+5) * 3.14 /180);

        Path mPath = new Path();
        mPath.moveTo(xTop, yTop);
        mPath.lineTo(xLeft, yLeft);
        mPath.lineTo(xRight, yRight);
        mBitmapCanvas.drawPath(mPath, mDrawTrianglePaint);
    }

    public void drawCircle(Canvas c) {
        MPPointF center = mChart.getCenterCircleBox();
        float radius = mChart.getRadius()+14f;
        float centerX = center.x;
        float centerY = center.y;
        mDrawCirclePaint.setStrokeWidth(28f);

        mBitmapCanvas.drawCircle(centerX, centerY, radius, mDrawCirclePaint);
    }
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        // 不单独画highlight， 在drawDataSet中画
        drawdHighlighted(c, indices);
    }

    public void drawdHighlighted(Canvas c, Highlight[] indices) {
